...
def Deal(sock, user,tran):
	...
	while True:
		...
		if(tran==True):
			...
			if(data=='q'):
				data='leave '+chatwith[0]+' chat'
				sock.send(data.encode())
				del chatwith[0]
				tran=False
			else:
				data= user + ' '+data
				clients[chatwith[0]].send(data.encode())
		else:
			data = sock.recv(BUFSIZ).decode()
			tmp=data.split()
...
while True:
	...
	if(data=='run'):
		...
		chat = threading.Thread(target = Deal, args = (tcpCliSock,name,tran))
		chat.start()
